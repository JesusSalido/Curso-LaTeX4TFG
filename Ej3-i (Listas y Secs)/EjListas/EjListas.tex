...
Personalmente pienso que \LaTeX{} proporciona demasiado espaciado en las listas. Con el paquete \texttt{enumitem} se puede compactar las listas. A continuación se muestra un ejemplo de entorno {\tt itemize} con la opción \texttt{nosep} y cambio de viñeta por defecto (proporcionada por el paquete \texttt{enumitem}):
...
\noindent Ejemplo de entorno {\tt enumerate} con cambio de numeración (letras minúsculas) y opción \texttt{nosep} (proporcionada por el paquete \texttt{enumitem}) para listas numeradas:
...
% Ejemplo:
% ============
\begin{enumerate}[\ding{52}] % Lista en la que se cambia el bullet por un símbolo dingbat
	\item peras
	\item manzanas
	\item naranjas
\end{enumerate}
