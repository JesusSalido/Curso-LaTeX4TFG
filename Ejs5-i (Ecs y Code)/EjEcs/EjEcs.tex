...
\usepackage{amsthm}		% Fuentes en entorno matemático
\usepackage{amssymb}	% Símbolos matemáticos (carga amsfonts)
%--- Tipografía (Opción 1)
...
\captionsetup[table]{skip=4pt} 	% Separación del caption en las tablas aumentando el
% valor por defecto 
...
\begin{compactitem}
	...
	\item \verb|\si{\highlight{red}\kilogram\cancel\metre\per\second}|: \si{\highlight{red}\kilogram\cancel\metre\per\second}
\end{compactitem}
...
A continuación se muestran algunos ejemplos de fórmulas químicas empleando el paquete \texttt{chemfig}. En la Fig.~\ref{fig:chemfig} se muestra como es posible tratar como una figura una fórmula generada con \texttt{chemfig}.


% Ejemplo: Cafeina
% ============
\begin{center}
	\chemfig{H_3C-[:30]N**6(-(=O)-(**5(-N(-CH_3)--N-))--N(-CH_3)-(=O)-)}\\[1cm]
\end{center}
...
% Ejemplo: 
% ============
% Este es un poco más complejo al que se ha añadido un escalado con 
% \resizebox para ajustarlo al tamaño de la línea. Esta solución
% escala también las fuentes.
\begin{figure}
\begin{center}
\resizebox{\textwidth}{!}{
...
\end{center}
\caption{Ejemplo de fórmula química hecha con \texttt{chemfig} e insertada como figura}
\label{fig:chemfig}
\end{figure}
